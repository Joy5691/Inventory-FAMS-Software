...
// Find all occurrences of {showChallanModal && challanTargetGRN && ( ... )} and remove them
// Since regex matching across multiple lines can be tricky and we have 2 copies now,
// let's do this carefully.

const removeModal = (sourceCode) => {
  let result = sourceCode;
  while (true) {
    const startIndex = result.indexOf('{showChallanModal && challanTargetGRN && (');
    if (startIndex === -1) break;

    // Find matching closing brace/parenthesis for the modal
    ...
    // Simplest way is to find the next `{/* RECEIVE CHALLAN MODAL */}` or similar,
    // but we can also just search for the end pattern.
    
    // Instead of regex, let's just strip everything from startIndex to the ')}' 
    // that follows the form.
    const endStr = '      )}\n'; // We know it ends with this. Let's find the first one after startIndex
    let endIndex = result.indexOf('</form>\n          </div>\n        </div>\n      )}', startIndex);
...
code = removeModal(code);
code = code.replace(/{showChallanModal && challanTargetGRN && \([\s\S]*?<\/form>\s*<\/div>\s*<\/div>\s*\)\}/g, '');
code = code.replace(/{\/\* RECEIVE CHALLAN MODAL \*\/}\s*/g, '');

const finalModalCode = `
...
if (insertIndex !== -1) {
  code = code.slice(0, insertIndex) + finalModalCode + '\n' + code.slice(insertIndex);
