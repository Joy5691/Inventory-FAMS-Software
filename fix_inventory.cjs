...
// The modal was injected before the export, but likely outside the component scope!
// We need to move it inside the component.

// 1. Remove the wrongly placed modal JSX from the end of the file
const startOfBadModal = "{showChallanModal && challanTargetGRN && (";
const endOfBadModal = "</button>\\n        </div>\\n      </div>\\n    </div>\\n  )}\\n";

const fileLines = code.split('\\n');
let newCodeLines = [];
let inBadModal = false;

for (let i = 0; i < fileLines.length; i++) {
  if (fileLines[i].includes('{showChallanModal && challanTargetGRN && (')) {
    inBadModal = true;
  }
  if (inBadModal) {
    if (fileLines[i].includes('Confirm & Receive Goods')) {
       // skip a few more lines to end of modal
    }
    if (fileLines[i].trim() === ')}') {
       inBadModal = false;
       continue;
    }
    continue; // skip lines while in bad modal
  }
  newCodeLines.push(fileLines[i]);
}

let cleanedCode = newCodeLines.join('\\n');

// A safer way: Let's just find the function signature and add the state, 
// then find the main return block and add the modal inside the main div wrapper.

// First, find and completely strip the previously injected modal and state to be clean
code = fs.readFileSync('src/pages/InventoryPage.tsx', 'utf-8');
code = code.replace(/\{showChallanModal && challanTargetGRN && \([\s\S]*?Confirm & Receive Goods[\s\S]*?<\/button>\n\s*<\/div>\n\s*<\/div>\n\s*<\/div>\n\s*\)\}/, '');


// Find the end of the component return statement to inject the modal correctly
const modalJSX = `
...
// Check if state is declared inside the component
if (code.includes('const [showChallanModal, setShowChallanModal] = useState(false);') && !code.match(/export default function InventoryPage\(\) \{[\s\S]*?const \[showChallanModal, setShowChallanModal\] = useState\(false\);/)) {
    // State is outside component. Clean it.
    code = code.replace(/const \[activeTab, setActiveTab\] = useState\('stocks'\);\s*const \[showChallanModal, setShowChallanModal\] = useState\(false\);\s*const \[challanTargetGRN, setChallanTargetGRN\] = useState<any>\(null\);\s*const \[challanData, setChallanData\] = useState\(\{[\s\S]*?\}\);/m, "");
    
    // Inject state safely into component
    code = code.replace(
...
} else if (!code.includes('const [showChallanModal, setShowChallanModal] = useState(false);')) {
     code = code.replace(
      /export default function InventoryPage\(\) \{/,
      `export default function InventoryPage() {
  const [activeTab, setActiveTab] = useState('stocks');
...
// Replace the ending tag of the main div
code = code.replace(
  /\{showNewMTVModal && \([\s\S]*?<\/div>\n    <\/div>\n  \);\n\}/,
  (match) => {
